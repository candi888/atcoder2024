...
def main() -> None:
    ...
    cur_r_set = SortedMultiset([])
    li_to_ri = defaultdict(SortedMultiset)
    for li, ri in query:
        li_to_ri[li - 1].add(ri - 1)
        cur_r_set.add(ri - 1)

    res = 0
    for curl in range(m):
        cur_ri = cur_r_set.ge(curl)

        if cur_ri == None:
            cur_ri = m
        res += cur_ri - curl

        for disri in li_to_ri[curl]:
            cur_r_set.discard(disri)

    if cur_r_set.__len__() != 0:
        raise RuntimeError
    print(res)
